...
public class InfoflowResultsReader {
	...
	public SerializedInfoflowResults readResults(String fileName) throws XMLStreamException, IOException {
		...
		try (InputStream in = new FileInputStream(fileName)) {
			...
			List<SerializedSourceInfo> sources = new ArrayList<>();
			...
			while (reader.hasNext()) {
				...
				if (reader.getLocalName().equals(XmlConstants.Tags.root) && reader.isStartElement()
					...
					category = getAttributeByName(reader, XmlConstants.Attributes.category);
					...
					// Clear the fields. allocate a new one, do not clear
					apFields = new ArrayList<>();
					...
					// Read the attributes
					srcStatement = getAttributeByName(reader, XmlConstants.Attributes.statement);
					srcMethod = getAttributeByName(reader, XmlConstants.Attributes.method);
					category = getAttributeByName(reader, XmlConstants.Attributes.category);
					// Clear the old state. allocate not clear
					...
					if (reader.getLocalName().equals(XmlConstants.Tags.accessPath))
						...
						sink = new SerializedSinkInfo(ap, statement, method, category);
					else if (reader.getLocalName().equals(XmlConstants.Tags.source)) {
						source = new SerializedSourceInfo(ap, srcStatement, srcMethod, category, pathElements);
						sources.add(source);//we need to add source here, or the source is overwritten afterwards
					}
					else if (reader.getLocalName().equals(XmlConstants.Tags.result)) {
						for(SerializedSourceInfo sourceinfo: sources) {
							results.addResult(sourceinfo, sink);
						}
					}
